        ...
        switch (mouseEvent.getType()){
            case MOUSE_UP:
                    ...
                    if(info != null){
                        if(info.getPlug() == null){
                            ...
                            if(info.getPlugLocation() == ProgramBlockInsertInfo.PlugLocation.BODY){
                                if(info.getSocket() instanceof FunctionBodyBlock){
                                    mediator.send(new ConnectContainerBlockBody((FunctionBodyBlock)info.getSocket(),(ReadOnlyStatementBlock)info.getPlug()));
                                ...
                                mediator.send(new ConnectConditionBlock((ReadOnlyControlFlowBlock) info.getSocket(), (ReadOnlyConditionBlock) info.getPlug()));
                            }else{
                ...
                if(programSelection != null){
                    ...
                    var info = getSocketAndPlug(recordedMouse,programSelection.getBlockType().getSource());
                    if(info != null){
                        if(info.getPlug() == null){
                            mediator.send(new AddBlock(info.getSocket()));
                            programArea.updateBlockProgram(info.getSocket());
